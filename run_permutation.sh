...
CANCER_DATA_DIR="/cs/labs/dina/ophirmil12/PathwayAtlas/results_and_graphs/scores/clinvar_reg_dis_ordered_prob-" + $DISTANCE_METRIC

# Get all CSV files into an array
mapfile -t FILES < <(find "$CANCER_DATA_DIR" -type f -name "*.csv")
...
CANCER_FILE="${FILES[$SLURM_ARRAY_TASK_ID]}"

echo "Processing file: $CANCER_FILE"

# Run your Python script with the file as parameter
python main.py --cancer_file "$CANCER_FILE" "$DISTANCE_METRIC"