...
def get_sub_df_for_pathway(cancer_name: str, pathway) -> pd.DataFrame:
    """
    @param cancer_name: short name of the cancer type.
    @param pathway: KEGG pathway ID.
    @return: DataFrame of mutations in the given pathway for the given cancer type.
    """
    pathway_file = pjoin("/cs/labs/dina/ophirmil12/PathwayAtlas/data/kegg/pathways/objects/", f"{pathway}.pickle")
    if os.path.exists(pathway_file):
        genes_dict = pd.read_pickle(pathway_file)
        genes = list(genes_dict.keys())

        cancer_file = pjoin(CANCERS_PATH, f"{cancer_name}{MUTATIONS_CSV_SUFFIX}")
        cancer_df = pd.read_csv(cancer_file)

        sub_df = cancer_df[
            cancer_df[KEGG_COL].apply(
                lambda s: any(g in s.split(',') for g in genes)
            )
        ]
        ...
        print(f"Pathway file {pathway_file} does not exist.")
        return pd.DataFrame()

def get_unique_statistic(cancer_name: str, all_pathways, statistic_type) -> int:
    """
    @param cancer_name: short name of the cancer type.
    @param all_pathways: list of all pathways to consider.
    @param statistic_type: type of statistic to count unique values for.
    @return: number of unique mutations in the given cancer type.
    """
    num_mutations = 0
    for p in all_pathways:
        sub_df = get_sub_df_for_pathway(cancer_name, p)
        if not sub_df.empty:
            num_mutations += sub_df[statistic_type].nunique()

    return num_mutations

def process_results(cbio: CbioApi) -> None:
    """
    ...
    for filename in all_files:
        # Read each file and add a 'cancer_type' column based on filename
        temp_df = pd.read_csv(filename)
        df_dict[filename] = temp_df

    for filename, df in df_dict.items():

        cancer_name = filename.split("/")[-1].replace(MUTATIONS_CSV_SUFFIX, "")
        cancer_type = cbio.get_cancer_type(cancer_name)

        num_significant_01 = df['significant_0.01'].sum()
        num_significant_05 = df['significant_0.05'].sum()

        all_pathways = df['pathway_name'].tolist()
        num_unique_mutations = get_unique_statistic(cancer_name, all_pathways, VARIANT_COL)
        num_patients = get_unique_statistic(cancer_name, all_pathways, PATIENT_ID_COL)

        num_samples = df['n'].sum()

        rows.append({
            'cancer_short_name': cancer_name,
            'cancer_type': cancer_type,
            'num_unique_mutations': num_unique_mutations,
            'num_samples_used': num_samples,
            'num_patients_used': num_patients,
            'num_candidate_pathways': len(df),
            'significant_pathways_0.01': num_significant_01,
            'significant_pathways_0.05': num_significant_05
        })

    num_sig_df = pd.DataFrame(rows)

    num_sig_df = num_sig_df.sort_values(by='significant_pathways_0.01', ascending=True)
    num_sig_df.to_csv(pjoin(LOTEM_RESULTS_PATH, "significant_pathways2.csv"), index=False)
